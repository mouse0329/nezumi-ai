...
**現在のバージョン**: v2.3.3（[リリースノート](docs/release-notes/v2.3.3.md)）
...
-   **OpenCL ノイズ問題との格闘**: 画像生成エンジン Nezumi Kiln で、GPU (OpenCL) 実行時に「中央だけ色がまとまり、周辺がランダムノイズ化した画像」が生成される問題に長期間取り組みました。チューニングヒューリスティック・演算精度 (FP16 オーバーフロー)・セッション解放タイミング・キャッシュ混在など複数の容疑者を一つずつ切り分けた結果、最終的な真因は **MNN の OpenCL バックエンド (low memory モード) が、int8 量子化された重みを int4 として解釈してしまい、読み込みビット幅が半分にズレてテンソルの後半が未初期化メモリで計算されていた** ことでした。数値的にはもっともらしい値が出るため NaN 検知をすり抜ける性悪なバグで、多段の診断コメントが [`mnn-sd-engine/src/mnn_session.cpp`](mnn-sd-engine/src/mnn_session.cpp) に記録されています。この切り分けの過程で得られた知見 (VAE の FP16 オーバーフロー対策、解像度別カーネルキャッシュ、クロスアテンションの NaN 検知など) はすべて現行コードに還元されています。
...
- **現在のバージョン**: v2.3.3（[リリースノート一覧](docs/release-notes/)）
- **主要コンポーネント**:
...
※ 初回推論時間は GGUF 重みのメモリ読み込み (mmap / ページイン) 時間を含みます。
  2B モデル (約2GB) の場合、実際のメモリ帯域速度で読み込むだけでも約10秒程度を要するため、
  以前掲げていた「2B < 3秒」は物理的に達成不可能な目標でした。4B は容量が約2倍のため
  さらに時間がかかります。なお2回目以降は OS のページキャッシュが効くため、初回より
  大幅に短縮されます。
