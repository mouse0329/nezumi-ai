...
class SettingsRepository(
    ...
    suspend fun saveCurrentSessionId(sessionId: Long) {
        val current = currentSettings()
        dao.update(current.copy(currentSessionId = sessionId))
    }

    suspend fun loadCurrentSessionId(): Long {
        val current = currentSettings()
    ...
    suspend fun getStopTokensForModel(modelPath: String): List<String> {
        val raw = currentSettings().stopTokensMap
        return parseStopTokensMap(raw)[modelPath] ?: emptyList()
    }

    suspend fun updateStopTokensForModel(modelPath: String, tokens: List<String>) {
        val current = currentSettings()
        val map = parseStopTokensMap(current.stopTokensMap).toMutableMap()
        if (tokens.isEmpty()) map.remove(modelPath) else map[modelPath] = tokens
        dao.update(current.copy(stopTokensMap = encodeStopTokensMap(map), lastModified = System.currentTimeMillis()))
    }

    private fun parseStopTokensMap(raw: String): Map<String, List<String>> {
        ...
        return raw.split('|')
            .mapNotNull { entry ->
                ...
                val tokens = entry.substring(idx + 1).split(',').map { it.trim() }.filter { it.isNotEmpty() }
                if (tokens.isEmpty()) null else path to tokens
            }.toMap()
    }

    private fun encodeStopTokensMap(map: Map<String, List<String>>): String =
        map.entries.joinToString("|") { (path, tokens) -> "$path=${tokens.joinToString(",")}" }
    ...
    suspend fun getChatHistoryLimit(): Int {
        return currentSettings().chatHistoryLimit
    }

    suspend fun updateChatHistoryLimit(limit: Int) {
        val current = currentSettings()
        dao.update(
            current.copy(
