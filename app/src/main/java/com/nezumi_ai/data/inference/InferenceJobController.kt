...
class InferenceJobController {
    ...
    suspend fun <T> launchInference(
        ...
        return try {
            ...
            task?.let {
                it.state = InferenceState.CANCELLING
                it.elapsedMs = System.currentTimeMillis() - it.startTimeMs
            }
            Log.d(TAG, "Inference task cancelled: taskId=$taskId")
            Result.failure(e)
        } catch (e: Exception) {
            val task = tasks[taskId]
            task?.let {
                it.state = InferenceState.FAILED
                it.elapsedMs = System.currentTimeMillis() - it.startTimeMs
            }
