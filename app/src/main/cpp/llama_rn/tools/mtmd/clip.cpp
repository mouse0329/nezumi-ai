...
struct clip_model_loader
{
        ...
        // modalities
        {
            ...
            if (kv_vision_flag < 0 || kv_audio_flag < 0)
            {
                for (int ti = 0; ti < n_tensors; ++ti)
                {
                    ...
                    // Single-file integrated Gemma-4 (e.g. Unsloth): vision tensors use names like
                    // per_layer_model_proj / per_layer_token_embd without a vision.* prefix.
                    if (kv_vision_flag < 0 &&
                        (std::strstr(tname, "per_layer_model_proj") != nullptr ||
                         std::strstr(tname, "per_layer_token_embd") != nullptr))
                    {
                        has_vision = true;
                    }
                ...
                {
                    LOG_INF("%s: inferred vision encoder (KV %s missing; vision.* and/or per_layer_* tensors)\n",
    ...
    // Unsloth / integrated Gemma-4V GGUF may omit clip.vision.* metadata; infer from tensor layout.
    void infer_gemma4v_vision_hparams_from_tensors(clip_hparams &hp)
    {
        auto *meta = ctx_meta.get();
        if (!meta)
        {
            return;
        }
        auto get_t = [&](const char *n) -> lm_ggml_tensor *
        {
            return lm_ggml_get_tensor(meta, n);
        };

        if (hp.n_embd <= 0)
        {
            const char *names[] = {"token_embd.weight", "v.token_embd.weight"};
            for (const char *n : names)
            {
                lm_ggml_tensor *t = get_t(n);
                if (t && lm_ggml_n_dims(t) >= 1 && t->ne[0] > 0)
                {
                    hp.n_embd = (int32_t)t->ne[0];
                    break;
                }
            }
        }
        ...
        if (hp.n_ff <= 0)
        {
            const char *names[] = {"blk.0.ffn_up.weight", "v.blk.0.ffn_up.weight"};
            for (const char *n : names)
            {
                lm_ggml_tensor *t = get_t(n);
                if (t && lm_ggml_n_dims(t) >= 2 && hp.n_embd > 0)
                {
                    if ((int32_t)t->ne[0] == hp.n_embd)
                    {
                        hp.n_ff = (int32_t)t->ne[1];
                    }
                    else if ((int32_t)t->ne[1] == hp.n_embd)
                    {
                        hp.n_ff = (int32_t)t->ne[0];
                    }
                    if (hp.n_ff > 0)
                    {
                        break;
        ...
        if (hp.n_head <= 0 && hp.n_embd > 0)
        {
            const char *names[] = {"blk.0.attn_q_norm.weight", "v.blk.0.attn_q_norm.weight"};
            for (const char *n : names)
            {
                lm_ggml_tensor *qn = get_t(n);
                if (qn && lm_ggml_n_dims(qn) >= 1 && qn->ne[0] > 0)
                {
                    const int head_dim = (int)qn->ne[0];
                    if (head_dim > 0 && hp.n_embd % head_dim == 0)
                    {
                        hp.n_head = hp.n_embd / head_dim;
                    }
                    break;
                }
            }
        ...
        {
            int max_il = -1;
            const int nt = lm_gguf_get_n_tensors(ctx_gguf.get());
            for (int ti = 0; ti < nt; ++ti)
            {
                ...
                const bool is_attn_output = std::strstr(tname, "attn_output.weight") != nullptr;
                const bool is_attn_out = std::strstr(tname, "attn_out.weight") != nullptr;
                if (!is_attn_output && !is_attn_out)
                {
                    continue;
                }
                int il = -1;
                if (std::sscanf(tname, "blk.%d.", &il) == 1)
                {
                    max_il = std::max(max_il, il);
                }
                else if (std::sscanf(tname, "v.blk.%d.", &il) == 1)
                {
                    max_il = std::max(max_il, il);
                }
            }
        ...
        if (hp.projection_dim <= 0)
        {
            lm_ggml_tensor *mm = get_t("mm.input_projection.weight");
            if (mm && lm_ggml_n_dims(mm) >= 2)
            {
                hp.projection_dim = (int32_t)mm->ne[1];
            }
            if (hp.projection_dim <= 0 && hp.n_embd > 0)
            {
                hp.projection_dim = hp.n_embd;
            }
        }
        ...
        {
            hp.patch_size = 14;
            LOG_INF("%s: inferred patch_size=%d (KV omitted; Gemma4V default)\n", __func__, hp.patch_size);
        }

        LOG_INF("%s: inferred vision hparams from tensors: n_embd=%d n_head=%d n_ff=%d n_layer=%d projection_dim=%d\n",
                __func__, hp.n_embd, hp.n_head, hp.n_ff, hp.n_layer, hp.projection_dim);
    }
    ...
    void load_hparams(clip_model &model, clip_modality modality)
    {
        ...
        std::string proj_type;
        {
            ...
            // Exporters (e.g. Unsloth) may omit clip.projector_type and clip.vision.projector_type.
            // Integrated Gemma-4V stacks use distinctive tensor names; infer when KV is empty.
            if (proj_type.empty() && modality == CLIP_MODALITY_VISION)
            {
                const int nt = lm_gguf_get_n_tensors(ctx_gguf.get());
                for (int ti = 0; ti < nt; ++ti)
                {
                    ...
                    if (std::strstr(tname, "per_layer_model_proj") != nullptr ||
                        std::strstr(tname, "per_layer_token_embd") != nullptr)
                    {
                        proj_type = "gemma4v";
                        LOG_INF("%s: inferred projector type '%s' (KV omitted; per_layer_* Gemma-4V tensors)\n",
        ...
        const bool gemma4v_sparse_kv =
            is_vision &&
            model.proj_type == PROJECTOR_TYPE_GEMMA4V &&
            lm_gguf_find_key(ctx_gguf.get(), string_format(KEY_N_EMBD, "vision").c_str()) < 0;

        // other hparams
        {
            const char *prefix = is_vision ? "vision" : "audio";
            if (gemma4v_sparse_kv)
            {
                get_u32(string_format(KEY_N_EMBD, prefix), hparams.n_embd, false);
                get_u32(string_format(KEY_N_HEAD, prefix), hparams.n_head, false);
                get_u32(string_format(KEY_N_FF, prefix), hparams.n_ff, false);
                get_u32(string_format(KEY_N_BLOCK, prefix), hparams.n_layer, false);
                get_u32(string_format(KEY_PROJ_DIM, prefix), hparams.projection_dim, false);
                get_f32(string_format(KEY_LAYER_NORM_EPS, prefix), hparams.eps, false);
                infer_gemma4v_vision_hparams_from_tensors(hparams);
            }
            else
            {
            ...
            if (is_vision)
            {
                if (gemma4v_sparse_kv)
                {
                    get_u32(KEY_IMAGE_SIZE, hparams.image_size, false);
            ...
            if (is_vision)
            {
                ...
                else if (gemma4v_sparse_kv || model.proj_type == PROJECTOR_TYPE_GEMMA4V)
                {
                    // Match HF Gemma vision preprocessing (RGB, resized then normalized)
                    static const float def_mean[3] = {0.485f, 0.456f, 0.406f};
                    static const float def_std[3] = {0.229f, 0.224f, 0.225f};
                    for (int i = 0; i < 3; ++i)
                    {
                        hparams.image_mean[i] = def_mean[i];
                        hparams.image_std[i] = def_std[i];
                    }
                    LOG_INF("%s: inferred image_mean/std (KV omitted; ImageNet-style Gemma4V defaults)\n", __func__);
                }
                else
                {
